...
class MockTransportSessionTeamTests : MockTransportSessionTests {
    ...
    func testThatItFetchesTeam() {
        ...
        let response = self.response(forPayload: nil, path: path, method: .methodGET, apiVersion: 0)
...
extension MockTransportSessionTeamTests {
    ...
    func testThatTeamConversationCantBeDeleted_ByNonTeamUser() {
        ...
        // When
        let response = self.response(forPayload: nil, path: "/teams/\(team.identifier)/conversations/\(conversation.identifier)", method: .methodDELETE, apiVersion: 0)
...
extension MockTransportSessionTeamTests {
    ...
    func testThatItFetchesTeamMembersByID() {
        ...
        let response = self.response(forPayload: requestPayload as ZMTransportData, path: path, method: .methodPOST, apiVersion: 0)
        XCTAssertNotNil(response)
        XCTAssertEqual(response?.httpStatus, 200)
        XCTAssertNotNil(response?.payload)
    ...
    func testThatItDoesNotApproveLegalHoldRequestForNonPendingUser() {
        ...
        let response = self.response(forPayload: ["password": "Ex@mple!"] as NSDictionary, path: path, method: .methodPUT, apiVersion: 0)
        XCTAssertNotNil(response)
