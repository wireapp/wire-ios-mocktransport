...
class MockServicesTests: MockTransportSessionTests {
    func testThatInsertedServiceCanBeQueried() {
        ...
        let response = sut.processTeamsRequest(ZMTransportRequest(path: "/teams/\(teamID.transportString())/services/whitelisted?prefix=Normal", method: .methodGET, payload: nil, apiVersion: .v0))
        // then
    ...
    func testThatItCanAddServiceToTheConversation() {
        ...
        let response = sut.processServiceRequest(ZMTransportRequest(path: "/conversations/\(conversation.identifier)/bots", method: .methodPOST, payload: payload as ZMTransportData, apiVersion: .v0))
    ...
    func testThatItCanFetchServiceById() {
        ...
        let _ = sut.insertService(name: "Normal Service",
                                  ...
                                  provider: providerId)
        // when
        let request = ZMTransportRequest(getFromPath: "/providers/\(providerId)/services/\(serviceId)", apiVersion: .v0)
    ...
    func testThatItCanFetchProviderById() {
        ...
        // when
        let request = ZMTransportRequest(getFromPath: "/providers/\(providerId)", apiVersion: .v0)
        let response = sut.processServicesProvidersRequest(request)
    ...
    func testThatItCanDeleteBotFromConversation() {
        ...
        let payload = ["service": service.identifier,
                       "provider": service.provider]
        let _ = sut.processServiceRequest(ZMTransportRequest(path: "/conversations/\(conversation.identifier)/bots", method: .methodPOST, payload: payload as ZMTransportData, apiVersion: .v0))
        ...
        // when
        let request = ZMTransportRequest(path: "/conversations/\(conversation.identifier)/bots/\(botUser.identifier)", method: .methodDELETE, payload: nil, apiVersion: .v0)
