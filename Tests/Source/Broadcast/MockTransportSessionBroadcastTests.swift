...
class MockTransportSessionBroadcastTests: MockTransportSessionTests {
    ...
    func testThatItReturnsMissingConnectedUsersWhenReceivingOTRMessage() {
        ...
        // when
        let responseJSON = self.response(forPayload: payload as ZMTransportData, path: "/broadcast/otr/messages", method: .methodPOST, apiVersion: 0)
        let responsePROTO = self.response(forProtobufData: protoPayload, path: "/broadcast/otr/messages", method: .methodPOST, apiVersion: 0)
