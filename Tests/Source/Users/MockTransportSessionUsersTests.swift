...
final class MockTransportSessionUsersTests_Swift: MockTransportSessionTests {
    ...
    func testThatItReturnsUserClientsKeys() {
        ...
        let response: ZMTransportResponse = self.response(forPayload: payload, path: "/users/prekeys", method: .methodPOST, apiVersion: 0)
    ...
    func testThatItReturnsRichInfo_404_whenUserDoesNotExist() {
        ...
        // when
        let response = self.response(forPayload: nil, path: "/users/\(userId)/rich-info", method: .methodGET, apiVersion: 0)
    ...
    func testThatItReturnsRichInfo_EmptyWhenUserDoesNotHaveAny() {
        ...
        // when
        guard let response = self.response(forPayload: nil, path: "/users/\(userId)/rich-info", method: .methodGET, apiVersion: 0) else { XCTFail(); return }
