...
extension MockTransportSessionTests {
    ...
    func response(forAssetData assetData: Data, contentType: String, path: String, apiVersion: Int32) -> ZMTransportResponse? {
        ...
        let result = sut.mockedTransportSession().attemptToEnqueueSyncRequest {
            let request = ZMTransportRequest.multipartRequest(withPath: path, imageData: assetData, metaData: [:], mediaContentType: contentType, apiVersion: 0)
